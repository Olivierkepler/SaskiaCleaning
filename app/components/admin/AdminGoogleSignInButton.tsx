...
type AdminGoogleSignInButtonProps = {
  callbackUrl?: string;
  className?: string;
};
...
export default function AdminGoogleSignInButton({
  ...
  async function handleClick() {
    ...
    try {
      const intent = await fetch("/api/admin/auth/intent", { method: "POST" });
      if (!intent.ok) {
        throw new Error("Could not start admin sign-in.");
      }
