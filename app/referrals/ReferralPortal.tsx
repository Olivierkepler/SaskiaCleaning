...
const GENERIC_LOOKUP_MESSAGE_KEY = "genericSuccess";
...
type ReferralPortalProps = {
  /** When true, omit full-page chrome for embedding in account shell. */
  embedded?: boolean;
};
...
export default function ReferralPortal({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("error" in data ? data.error : t("requestFailed"));
      }

      setSubmitSuccess(
        "message" in data ? data.message : t(GENERIC_LOOKUP_MESSAGE_KEY),
      );
    } catch (lookupError) {
      setSubmitError(
  ...
  const body = (
    <div className={embedded ? "relative" : "relative mx-auto max-w-4xl"}>
      {!embedded ? (
        <Link
          href="/"
